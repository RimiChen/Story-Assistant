...
class Story(object):
    ...
    def get_scenes(self):
        return [v for k,v in list(self.scenes.items())]
    ...
    def get_storypoints(self, sceneNum):
        try:
            return [v for k,v in list(self.get_scene(str(sceneNum)).points.items())]
        except:
            return []
    ...
    def realize(self):
        for scene,v in sorted(self.scenes.items()):
            if v.header:
                print((v.header))
            if v.setting:
...
def get_points(brain,size):
    ...
    for k,v in list(allPoints.items()):
