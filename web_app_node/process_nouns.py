...
def process_json(print_limit, noun_list):
    new_json_file = "./text_sample/"+str(sys.argv[1]).replace(".txt","")+".json"
    ...
    for key in noun_list:
        if key_count < print_limit:
            new_dict[key] = noun_list[key]
        key_count = key_count+1; 
        #print key, noun_list[key]
    target.write(json.dumps(new_dict, sort_keys=False, indent=4, separators=(',', ': ')))
    #print (new_json_file)
    #print ()
...
name_list = process_content_find_verb(int(sys.argv[2]))
process_json(int(sys.argv[2]), name_list)